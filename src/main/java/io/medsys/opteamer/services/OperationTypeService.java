...
import io.medsys.opteamer.mapper.PreOperativeAssessmentMapper;
...
public class OperationTypeService {
    ...
    private void setChildEntities(OperationTypeDTO operationTypeDTO, OperationType operationType) {
        ...
        operationTypeDTO.getAssets().stream().forEach(assetDTO -> {
            Asset asset = assetRepoistory.findById(assetDTO.getId()).get();
            assets.add(asset);
        });
        ...
        Set<PreOperativeAssessment> preOperativeAssessments = operationTypeDTO.getPreOperativeAssessments().stream()
                .map(preOperativeAssessmentDTO -> preOperativeAssessmentRepository.findByName(preOperativeAssessmentDTO.getName())
                        .orElseThrow(() -> new NoSuchElementException())).collect(Collectors.toSet());
